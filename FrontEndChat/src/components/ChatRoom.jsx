...
export const ChatRoom = ({messages, roomName, closeChat, sendMessage, userName}) => {
    ...
    <div className="w-1/2 bg-white p-8 rounded shadow-lg">
        <div className="flex flex-row justify-between mb-5">
            <Heading size={"lg"}>Room name: {roomName}</Heading>
            <CloseButton onClick={() => closeChat()}/>
        </div>
        <div>
        <Heading size={"m"}>User name: {userName}</Heading>
        </div>
